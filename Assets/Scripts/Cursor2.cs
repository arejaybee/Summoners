...
public class Cursor2 : MonoBehaviour {
    ...
    private int MAX_Z = 0;
    public string[] summonNames = {"Fairy", "Griffon", "Minotaur", "Gorgon", "Centaur","Knight","Pegasus", "Werewolf", "Dragon"};
    public int summonOffset = 0;
	public bool tryingToSummon;

    // Use this for initialization
    void Start()
    {
		tryingToSummon = false;
    ...
    void checkConfirm()
    {
		...
		Mana manaVal = GameObject.Find(sumName).transform.FindChild("Mana").GetComponent<Mana>();

        //check if the summoner can afford the piece
        afford = canAfford(newPiece, manaVal.manaValue);
        if (afford)
        {
			onSummonScreen = false;

			//swap to trying to place unit on the map
			GameObject.Find("Cursor3").GetComponent<Cursor3>().placeUnit(ref newPiece);
		}
        else
        {
			Destroy(newPiece);
     ...
     */
    bool canAfford(GameObject chara, int theirMana)
    {

        //check if they have enough mana here and subtract that mana
        if (theirMana < chara.GetComponent<Character>().getCost())
        {
			return false;
        }
        return true;
